<!DOCTYPE html>
<html lang="fr">
	<head> <!-- Entête de la page -->
        <!-- Google tag (gtag.js) --> <script async src="https://www.googletagmanager.com/gtag/js?id=G-4XVJRDS582"></script> <script> window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-4XVJRDS582'); </script>
		<title>Cécile PETITJEAN - Kinésiologue - Qui suis-je ?</title>
        <meta name="description" content="Présentation et parcours de Cécile PETITJEAN, kinésiologue certifiée. Kinésiologie et aides aux apprentissages. Réservation en ligne.">

        <?php include('meta.php'); ?>
	</head>

	<body> <!-- Corps de la page -->
        <div id="bloc_page"> <!--page complète-->
            <header class ="header">
                <?php include('header.php'); ?>
                <nav class="header_nav">
                    <ul>
                        <li><a href="index.html" title="Accueil">Accueil</a></li>
                        <li><a href="p2_kinesiologue.php" title="La kinésiologie, c'est quoi ?">Kinésiologie</a></li>
                        <li><a href="p3_apprentissage.php" title="L'aide aux apprentissages">Apprentissages</a></li>
                        <li class="header_nav-active"><a href="p4_qui.php" title="Qui suis-je ?">Qui suis-je ?</a></li>
                        <li><a href="p5_sceance.php" title="Comment se déroule une séance de kinésiologie ?">Séance</a></li>
                        <li><a href="p6_contacts.php" title="Informations pratiques">Contacts, Tarif</a></li>
                    </ul>
                </nav>
            </header>    
            
            <div class="banniere"> <!--Bannière-->
                <p>Il n'est jamais trop tard pour éclore !</p>
            </div>

            <section>
                <?php @include('section_nav.php'); ?> <!-- bandeau lateral de navigation -->
                
                <article> 
                    <div class="qui">
                        <h2>Qui suis-je ?</h2>
                        <figure class="figure_left">
                            <img class="figure_photo" src="images/cecile_ext.jpg" alt="Cécile PETITJEAN photo" title="Cécile PETITJEAN - Kinésiologue"/>
                            <figcaption>&copy; Cécile Petitjean Kinésiologue</figcaption>
                        </figure>
                        <p>D'abord enseignante dans l’Education Nationale pendant plus de 20 ans, j’ai enseigné dans les différents niveaux de l’école primaire, de la maternelle à l’élémentaire et dans différentes écoles, toujours avec beaucoup de passion.</p>
                        <p>J’ai toujours eu à cœur de faire réussir les enfants qui m’étaient confiés. J’ai beaucoup aimé mon métier, mais je sentais bien que quelque chose manquait dans le fonctionnement de l’école, que les <strong>potentiels individuels</strong> de chaque enfant n’étaient pas suffisamment explorés et mis en valeur, que les intelligences multiples et variées ne pouvaient pas s’exprimer pleinement.</p>
                        <p>Année après année, il m’est apparu comme une évidence qu’il fallait davantage prendre en considération l’enfant dans son individualité mais aussi et surtout dans sa <strong>globalité</strong> pour pouvoir réussir à lui enseigner quelque chose ; que la mauvaise, ou même l'absence de prise en considération des émotions, des ressentis, des histoires de vie, limitait forcément la possibilité d’<strong>exprimer tout leur potentiel</strong>.</p>
                        <p>Aussi j’ai d’abord cherché à me former dans la <strong>gestion des émotions</strong>.</p>
                        <p>Très vite il m’est apparu que cela n’était pas suffisant. Nos <strong>réactions émotionnelles</strong> étant étroitement liées à notre <strong>histoire</strong>, il fallait aussi la prendre en considération pour mieux comprendre et maîtriser notre fonctionnement actuel, mieux <strong>gérer notre stress</strong>, nos <strong>émotions</strong>, avoir des <strong>projets</strong>, des <strong>ambitions</strong>, se sentir en <strong>confiance avec soi-même et avec les autres</strong>.</p>
                        <p>J’ai cherché à découvrir des méthodes de <strong>bien être</strong> et j’ai dévouvert et testé la <strong>kinésiologie</strong> à titre personnel, pour moi et pour mes propres enfants.<br /> Cette approche a particulièrement retenu mon attention par sa pertinence, sa précision et son efficacité, et par son <strong>approche holistique</strong>, c'est à dire en considérant la personne dans sa globalité.</p>
                        <figure class="figure_right_large2">
                            <img class="figure_photo" src="images/diplome.jpg" alt="Certification de Kinésiologue" title="Certification de Kinésiologue - Cécile PETITJEAN"/>
                            <figcaption>&copy; Cécile Petitjean Kinésiologue</figcaption>
                        </figure>
                        <p>J'ai beaucoup lu et appris sur le sujet et j'ai décidé alors de me former à l’<a href="https://www.ecole-kinesiologie.fr/"  target="_blank" title="Lien vers le site de l'EKTC">EKTC</a> (<strong>Ecole de Kinésiologie et Techniques Complémentaires</strong>) pendant 2 ans.<br />Suite à ce long travail d'apprentissage et de mises en applications, j'ai été officiellement certifiée en tant que <strong>Kinésiologue</strong>.</p>
                        <p>Cette double casquette, d'<strong>enseignante</strong> et de <strong>kinésiologue</strong>, me permet aujourd'hui de bien comprendre l'impact des émotions et de l'histoire de vie sur les capacités d'apprentissage et le bien-être des enfants, mais aussi des adultes qu'ils ont pu devenir.</p>
                        <p>J'interviens, grâce à la kinésiologie, sur <strong>l'émotionnel</strong> et <strong>l'estime de soi</strong>, afin de trouver un mieux être et un équilibre.<br />Après ce rééquilibrage des émotions, je peux aider à remédier aux <strong>difficultés d'apprentissages</strong> en proposant une aide (scolaire) personnalisée.</p>
                        <p>Un soutien scolaire ou une remédiation spécifique des apprentissages (par exemple avec un orthophoniste ou un psychomotricien), ne pourra être efficace que si le socle est solide. Il faut faire les choses dans l'ordre !</p>
                        <p>Je peux également accompagner les adolescents et les parents rencontrant des difficultés d'ordre personnel ou en lien avec leur <strong>parentalité</strong>.</p>
                        <p><br />Que vous soyez <strong>enfant</strong>, <strong>adolescent</strong>, <strong>adulte</strong>, <strong>parent</strong>, je me tiens donc à votre disposition et vous propose des séances de kinésiologie ou d'aide aux apprentissages sur rendez-vous à mon cabinet.</p> 
                        <h3>N'hésitez pas à m'appeler (06 40 08 13 82) ou à m'envoyer un <a href="mailto:cecilepetitjeankinesiologue@gmail.com" title="Envoyer un email à cecilepetitjeankinesiologue@gmail.com">mail</a> pour plus d'informations</h3>
                        <h3>Pour prendre rendez-vous en ligne, cliquez sur ce lien :<br /> <a href="https://cecilepetitjeankinesiologue.simplybook.it/v2/" target="_blank" title="Prendre rendez-vous en ligne">Prendre Rendez-vous</a></h3>
                        <h4>Cécile</h4>
                    </div>
                    <?php include('calendrier.php'); ?>
                </article>
                    
                </div>
        </section>
                    
        <?php include('footer.php'); ?>
    </div>
    <?php include('gototop.php'); ?>
</body>
</html>

